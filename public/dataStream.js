/**
 * ============================================================================
 * ADAPTED HACKATHON TELEMETRY PIPELINE ENGINE (dataStream.js)
 * ============================================================================
 * Adapted for automation_projects.csv dataset.
 * Original fields remapped:
 *   annual_revenue_usd -> annual_savings_usd
 *   employee_count -> robots_deployed
 *   customer_count -> employee_hours_saved
 *   market_share_percent -> roi_percent
 *   founded_year -> removed
 * ============================================================================
 */
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (values.length === headers.length) {
        ...
        headers.forEach((header, index) => {
          ...
          if (['annual_savings_usd', 'robots_deployed', 'employee_hours_saved', 'budget_usd'].includes(header)) {
      ...
      setInterval(() => {
        ...
        for (let i = 0; i < batchSize; i++) {
          ...
          if (isAnomaly) {
            ...
            row.roi_percent = parseFloat((row.roi_percent + randomRange(-15, 15)).toFixed(2));
            // 10% of anomaly cases set project_status to Failed for alert triggers
            if (Math.random() < 0.1) {
              row.project_status = 'Failed';
            }
          } else {
            // High-frequency standard operational telemetry noise
            row.annual_savings_usd += Math.floor(randomRange(-50000, 100000));
          ...
          row.roi_percent = parseFloat(row.roi_percent.toFixed(2)); // Allow negative for alert trigger
